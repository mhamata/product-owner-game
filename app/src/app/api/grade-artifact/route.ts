 ...
 *
 * THE GRADING CONTRACT LIVES IN `@/lib/artifactGrader`, shared with the Phase-0
 * calibration harness (`scripts/calibrate.ts`) so the calibration study and
 * production grade identically. This route owns only the guardrails around it:
 *
...
/* ------------------------------------------------------------------
   ...
   token ceilings live with the grading core in @/lib/artifactGrader.
   ------------------------------------------------------------------ */
...
export async function POST(request: Request) {
  ...
  let reservedCents = 0;
  if (userId) {
    reservedCents = estimateCallCents(GRADER_MODEL, buildUserContent(body).length, MAX_OUTPUT_TOKENS);
  ...
  try {
    ...
    if (userId) {
      const actual = actualCallCents(GRADER_MODEL, usage.inputTokens, usage.outputTokens);
