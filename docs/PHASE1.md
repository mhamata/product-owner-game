...
## Next actions
1. Slice D (Stripe — needs Mike's account + price IDs): checkout route, webhook → `entitlements`, flip `PRAXIS_AUTH_MODE=required` for payers. The only remaining Phase-1 build.
